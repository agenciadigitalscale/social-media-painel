/* MinhaProducaoDesigner — o painel do próprio designer, no Meu Dia.

   Mostra SÓ os números do designer logado: quantas artes ele teve aprovadas
   hoje, na semana e no mês, mais o que está aguardando aprovação e em correção.
   Nada de comparação nem de outro designer — a conta é feita só sobre as artes
   dele (`artesDoDesigner(currentUser)`), então não há como um ver o do outro
   por aqui.

   A conta vem de `lib/designerProducao` (a MESMA da área administrativa), então
   o número que o designer vê é exatamente o que a gestão vê. Divergir aqui seria
   a pior surpresa possível — o designer conferindo um total diferente do fecho.
*/
...
export default function MinhaProducaoDesigner({ items, states, currentUser, now }: Props) {
    ...
    <Paper sx={{
      ...
      <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.4, mb: 1.6 }}>
        ...
        <Box sx={{ minWidth: 0 }}>
          ...
          <Typography sx={{ fontSize: { xs: '0.63rem', xl: '0.72rem' }, color: DS.t3 }}>
            {NAME_MAP[currentUser]?.emoji} {getDisplayName(currentUser)} — suas artes aprovadas
          </Typography>
        </Box>
      </Box>
      ...
      <Box sx={{ display: 'flex', gap: { xs: 1.5, md: 2.5 }, flexWrap: 'wrap', mb: porCliente.length > 0 || recentes.length > 0 ? 2 : 0 }}>
        <Metrica rotulo="Hoje" valor={resumo.aprovadasHoje} cor={resumo.aprovadasHoje > 0 ? DS.green : DS.t3}
          detalhe={resumo.aprovadasHoje === 1 ? '1 arte aprovada' : `${resumo.aprovadasHoje} aprovadas`} />
      ...
      {/* ── Produção recente (só as próprias artes) ── */}
      {recentes.length > 0 && (
      ...
      {resumo.aprovadasTotal === 0 && resumo.aguardando === 0 && resumo.correcao === 0 && (
        <Typography sx={{ fontSize: '0.72rem', color: DS.t3, mt: 0.5 }}>
          Nenhuma arte aprovada ainda. Quando o cliente aprovar suas artes, elas aparecem aqui.
        </Typography>
      )}
    </Paper>
  )
