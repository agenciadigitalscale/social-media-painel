 ...
 *
 * A moldura da mídia é o `MediaPreview` compartilhado (proporção, poster pela
 * service account, dica de mime, descarregar ao sair) — a mesma que o card usa.
 */
export default function ReviewModal({
  open, clientName, title, fileId, filename, mimeType, contentType,
  onApprove, onRequestFix, onOpenWhatsApp, onClose,
}: Props) {
  // Sem efeito de reset: o modal só existe enquanto está aberto (o pai o
  // renderiza dentro de `{reviewModal && …}`) e vem com `key={fileId}`, então
  // abrir de novo — ou trocar de arquivo — já monta um componente novo, com as
